...
void run_tile_conv(cl_param* cl_gpu, config* data, tile_param* tile, size_t* global_work_size, size_t* local_work_size)
{
	...
	prepare_to_execute(cl_gpu, false, "conv.cl", "convolution_v2");
...
void run_direct_conv(cl_param* cl_gpu, config* data, size_t* global_work_size, size_t* local_work_size)
{
	float* I = create_data(total_value(data, 'I'));
	float* W = create_data(total_value(data, 'W'));
	float* O = create_data(total_value(data, 'O'));
	...
	prepare_to_execute(cl_gpu, false, "direct_conv.cl", "seq_convolution");
	...
	int partsum_num = data->output_c*data->output_size*data->output_size*data->input_c*data->weight_size*data->weight_size;
	printf("partsum_num=%d\n", partsum_num);
	float* gpu_partsum = malloc_gpu_space(cl_gpu, sizeof(float)*partsum_num);
	float* gpu_O = malloc_gpu_space(cl_gpu, sizeof(float)*total_value(data, 'O'));
	...
	clock_t start, end;
	start = clock();
	direct_conv(cl_gpu, gpu_I, gpu_W, gpu_partsum, gpu_O, data, global_work_size, local_work_size);
	end = clock();
	printf("clock = %ld\n", (end-start));
...
int main(int argc, char* argv[])
{
	...
	// run_direct_conv(cl_gpu, data, &global_work_size, &local_work_size);
